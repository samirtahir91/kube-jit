...
func init() {
	...
	utilruntime.Must(jitv1.AddToScheme(scheme))
...
func main() {
	...
	flag.StringVar(
		&configurationName,
		"configuration-name",
		"kube-jit-operator-default",
		"name of the KubeJitConfig",
	)
	...
	if len(configurationName) == 0 {
		setupLog.Error(fmt.Errorf("missing KubeJitConfig configuration resource name"), "unable to start manager")
		os.Exit(1)
	}
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "fae37005.kubejit.io",
	...
	// Check for KUBE_JIT_OPERATOR_CONFIG_PATH environment variable and override configCacheFilePath
	if customCachePath := os.Getenv("KUBE_JIT_OPERATOR_CONFIG_PATH"); customCachePath != "" {
	...
	if err = (&controller.JitRequestReconciler{
		...
		Recorder: mgr.GetEventRecorderFor("jitrequest-controller"),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "JitRequest")
		os.Exit(1)
	}
	if err = (&config.KubeJitConfigReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
